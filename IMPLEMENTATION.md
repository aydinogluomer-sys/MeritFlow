...
> Son güncelleme: 2026-09-13 (Phase P2 1-B health risk-acceptance workflow + policy comparison — §3.8/§2.7/§3.11; append-only AUDITED waiver (reason+actor+timestamp+optional-expiry, never silently dismissed), authenticated INSERT gated by policy.manage+self, migration 0047 policy_health_risk_acceptances RLS ENABLE+FORCE + immutable + same-org FKs + DB-enforced version↔evaluation linkage; deterministic comparison-to-previous (overall+per-dimension deltas); accepting never changes score/policy/ledger, no LLM; reuses policy.manage (catalog 23); revocation OUT of scope; adversarial review fixed 1 finding; verify:fast (unit 592) + verify:contracts green; DB gates + 0047 type regen deferred to Linux CI. Second slice of Module 1; UI 1-C not built).
